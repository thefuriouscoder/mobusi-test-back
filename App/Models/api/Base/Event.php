...
/**
 * Base class that represents a row from the 'public.event' table.
 *
 *
 *
* @package    propel.generator.api.Base
*/
abstract class Event implements ActiveRecordInterface
{
    ...
    public function setPicture($v)
    {
        ...
        if ($this->picture !== $v) {
            $this->picture = $v;
            $this->modifiedColumns[EventTableMap::COL_PICTURE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : EventTableMap::translateFieldName('VenueId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->venue_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : EventTableMap::translateFieldName('Picture', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('public.event_id_seq')");
        ...
        if ($this->isColumnModified(EventTableMap::COL_PICTURE)) {
        ...
        $sql = sprintf(
            'INSERT INTO public.event (%s) VALUES (%s)',
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(EventTableMap::COL_PICTURE)) {
            $criteria->add(EventTableMap::COL_PICTURE, $this->picture);
        }
        if ($this->isColumnModified(EventTableMap::COL_DESCRIPTION)) {
