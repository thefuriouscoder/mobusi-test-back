...
use \Venue as ChildVenue;
use \VenueQuery as ChildVenueQuery;
use \Exception;
use \PDO;
use Map\VenueTableMap;
...
/**
 * Base class that represents a query for the 'public.venue' table.
 *
 *
 *
 ...
 *
 * @method     ChildVenue findOne(ConnectionInterface $con = null) Return the first ChildVenue matching the query
 ...
 *
 * @method     ChildVenue findOneById(int $id) Return the first ChildVenue filtered by the id column
 * @method     ChildVenue findOneByName(string $name) Return the first ChildVenue filtered by the name column *
...
abstract class VenueQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Base\VenueQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'mobusi', $modelName = '\\Venue', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = VenueTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT id, name FROM public.venue WHERE id = :p0';
        try {
